...
Usage:
    python scripts/figures/generate_figures.py --dataset all
"""
...
# =============================================================================
# Figure 2: Benchmark Percentiles Heatmap
# =============================================================================

def plot_benchmark_heatmap(dataset: str = "obscare", kpi: str = "target_rate"):
    """
    Create heatmap of benchmark percentiles across methods.
    Replaces bar chart where 0% values were invisible.
    """
    print(f"\n=== Generating Benchmark Heatmap ({dataset}, {kpi}) ===")
    
    csv_path = Path(f"results/{dataset}/tables/benchmark_results.csv")
    ...
    # Filter to target KPI only
    df = df[df['kpi'] == kpi].copy()
    ...
    hm = sns.heatmap(pivot, annot=True, fmt='.0f', cmap='RdYlGn', center=50,
    ...
    ax.set_title(f'Benchmark Percentiles: {kpi.replace("_", " ").title()} ({dataset}, N={len(pivot)})', 
    ...
    plt.tight_layout()
    output_path = FIGURES_DIR / f"benchmark_percentiles_{kpi}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved: {output_path}")
...
# =============================================================================
# Figure 3: Peer vs Global Percentile (P3.1/m7)
# =============================================================================
...
def main():
    ...
    parser.add_argument("--figures", default="all",
                       choices=["all", "privacy", "heatmap", "peer_global", "stability", "multiseed"],
    ...
    if args.figures in ["all", "heatmap"]:
        for ds in datasets:
            try:
                plot_benchmark_heatmap(ds)
            except Exception as e:
                print(f"  Heatmap error for {ds}: {e}")
