...
from src.utils import ensure_dir, get_logger, load_config, save_csv, save_json, set_seed, timer
...
def run_pipeline(config_path: str, privacy_epsilon: float = None) -> None:
    ...
    with timer("Data loading and preprocessing", logger) as t_preprocess:
        df = load_dataset(cfg)
        reps = prepare_representations(df, cfg)
    runtime_breakdown["preprocessing_seconds"] = t_preprocess["elapsed_seconds"]
    ...
    with timer("K-sweep (consensus clustering + perturbation eval)", logger) as t_ksweep:
        k_summary, cache = _k_sweep(df, reps, cfg)
        selected_k = _choose_k(k_summary, cfg)
    runtime_breakdown["k_sweep_seconds"] = t_ksweep["elapsed_seconds"]
    ...
    runtime_breakdown["total_seconds"] = pipeline_end - pipeline_start
    logger.info("Pipeline completed in %.2f seconds", runtime_breakdown["total_seconds"])
