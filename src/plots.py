...
def plot_confidence_histogram(
    ...
    """
    Plot histogram of per-institution confidence scores.

    Confidence scores measure how reliably each institution is assigned
    to its peer group across bootstrap iterations. Lower scores indicate
    unstable assignments that may need manual review.

    Parameters
    ----------
    confidence : np.ndarray
        1D array of confidence scores in [0, 1].
    path : str
        Output file path.
    threshold : float
        Threshold line to draw (default 0.5). Institutions below this
        threshold have uncertain peer assignments.
    title : Optional[str]
        Custom title (default: "Distribution of Cluster Confidence Scores").
    institution_ids : Optional[List[str]]
        If provided, annotates institutions with confidence below threshold.
    """
    fig, ax = plt.subplots(figsize=(7, 4.5))

    # Histogram
    n_bins = min(20, len(confidence))
    ...
    for patch, left_edge in zip(patches, bins[:-1]):
        if left_edge + (bins[1] - bins[0]) / 2 < threshold:
            patch.set_facecolor(COLORS["quaternary"])
            patch.set_alpha(0.7)
    ...
    below_threshold = np.sum(confidence < threshold)

    stats_text = (
        f"Mean: {mean_conf:.3f}\n"
        f"Median: {median_conf:.3f}\n"
        f"Below threshold: {below_threshold}/{len(confidence)}"
    )
    ax.text(
        0.02,
        0.98,
    ...
    ax.set_title(title or "Distribution of Cluster Confidence Scores")
    ax.set_xlim(0, 1.05)
    ax.legend(loc="upper right", framealpha=0.9)
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
...
def plot_multi_kpi_scatter(
    ...
    """
    Plot scatter of two KPI percentiles with cluster coloring.

    Shows how institutions rank on two different KPIs and whether
    peer groups capture both dimensions.

    Parameters
    ----------
    df : pd.DataFrame
        Benchmark results with percentile columns.
    kpi_x, kpi_y : str
        KPI names for x and y axes.
    labels : np.ndarray
    ...
    title : Optional[str]
        Custom title.
    institution_ids : Optional[List[str]]
        If provided, annotates points with institution names.
    """
    fig, ax = plt.subplots(figsize=(7, 6))
    ...
    for i, cluster in enumerate(unique_labels):
        ...
        if institution_ids is not None:
            ...
            # Extract column values as arrays for safe indexing
            x_col = df[kpi_x].values
            y_col = df[kpi_y].values
            for idx in mask_indices:
                x_val = float(x_col[idx])
                y_val = float(y_col[idx])
                ax.annotate(
                    institution_ids[idx],
                    (x_val, y_val),
                    fontsize=7,
    ...
    ax.plot([0, 100], [0, 100], "k--", linewidth=1, alpha=0.3, label="y=x")

    ax.set_xlabel(f"{kpi_x.replace('_', ' ').title()} Percentile")
    ax.set_ylabel(f"{kpi_y.replace('_', ' ').title()} Percentile")
    ax.set_title(title or f"Multi-KPI Scatter: {kpi_x} vs {kpi_y}")
    ax.set_xlim(-5, 105)
    ax.set_ylim(-5, 105)
    ax.legend(loc="upper left", framealpha=0.9)
    ax.set_aspect("equal")
...
def generate_all_figures(
    ...
    if "confidence" in results:
        path = str(output_path / "fig_confidence_histogram.png")
        plot_confidence_histogram(
            results["confidence"],
            path,
            institution_ids=results.get("institution_ids"),
        )
        generated.append(path)
