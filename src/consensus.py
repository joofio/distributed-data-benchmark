...
from src.clustering import cluster_data, gower_distance_matrix
...
def run_consensus(
    ...
    if representation in ("mixed_separated", "gower_pam", "kprototypes", "hierarchical_gower"):
        X_num, X_cat = X
        n = X_num.shape[0]
    else:
        n = X.shape[0]
    ...
    if representation == "numeric":
        ...
        iqr = np.subtract(*np.percentile(X, [75, 25], axis=0))
        iqr = np.where(iqr == 0, 1.0, iqr)
    elif representation in ("mixed_separated", "gower_pam", "kprototypes", "hierarchical_gower"):
    ...
    for b in range(n_boot):
        ...
        if representation == "numeric":
        ...
        elif representation == "gower_pam":
            # Gower distance + PAM: principled mixed-type clustering
            Xb_num = X_num[unique_idx].copy()
            Xb_cat = X_cat[unique_idx].copy()
            if feature_bootstrap:
                cols_num = _feature_bootstrap(rng, Xb_num.shape[1])
                ...
                Xb_num = Xb_num[:, cols_num]
                Xb_cat = Xb_cat[:, cols_cat]
            # Apply jitter before computing Gower distance
            Xb_num = _apply_numeric_jitter(Xb_num, rng, jitter_scale, iqr)
            # Compute Gower distance and cluster with PAM
            D = gower_distance_matrix(Xb_num, Xb_cat)
            labels = cluster_data((Xb_num, Xb_cat), k, seed + b, "pam", distance_matrix=D)
            assignments.append((unique_idx, labels))
            _update_coassignment(coassign, counts, labels, unique_idx)
        elif representation == "kprototypes":
            # K-prototypes: efficient mixed-type clustering
            Xb_num = X_num[unique_idx].copy()
            Xb_cat = X_cat[unique_idx].copy()
            if feature_bootstrap:
                cols_num = _feature_bootstrap(rng, Xb_num.shape[1])
                ...
                Xb_num = Xb_num[:, cols_num]
                Xb_cat = Xb_cat[:, cols_cat]
            # Apply jitter to numeric features only
            Xb_num = _apply_numeric_jitter(Xb_num, rng, jitter_scale, iqr)
            labels = cluster_data((Xb_num, Xb_cat), k, seed + b, "kprototypes", gamma=gamma)
            assignments.append((unique_idx, labels))
            _update_coassignment(coassign, counts, labels, unique_idx)
        elif representation == "hierarchical_gower":
            # Hierarchical clustering on Gower distance
            Xb_num = X_num[unique_idx].copy()
            Xb_cat = X_cat[unique_idx].copy()
            if feature_bootstrap:
                cols_num = _feature_bootstrap(rng, Xb_num.shape[1])
            ...
            # Apply jitter before computing Gower distance
            Xb_num = _apply_numeric_jitter(Xb_num, rng, jitter_scale, iqr)
            # Compute Gower distance and cluster with hierarchical
            D = gower_distance_matrix(Xb_num, Xb_cat)
            labels = cluster_data((Xb_num, Xb_cat), k, seed + b, "hierarchical", distance_matrix=D)
            assignments.append((unique_idx, labels))
            _update_coassignment(coassign, counts, labels, unique_idx)
        else:
