...
def validate_kpi_exclusion(cfg: Dict[str, Any]) -> None:
    """
    Validate that KPI columns are not included in clustering features.

    This prevents circularity in benchmarking: peer groups should be formed
    based on institutional characteristics, not performance outcomes.

    Raises
    ------
    ValueError
        If any KPI column is found in numeric or categorical feature lists.
    """
    numeric_cols = set(cfg["features"].get("numeric", []))
    categorical_cols = set(cfg["features"].get("categorical", []))
    kpi_cols = set(cfg["targets"]["kpis"])

    feature_cols = numeric_cols | categorical_cols
    overlap = feature_cols & kpi_cols

    if overlap:
        raise ValueError(
            f"KPI columns must not be included in clustering features to prevent "
            f"circularity. Found KPIs in feature lists: {sorted(overlap)}. "
            f"Remove these from 'features.numeric' or 'features.categorical' in config."
        )
...
def prepare_representations(df: pd.DataFrame, cfg: Dict[str, Any]) -> Representations:
    ...
    Validates that KPI columns are excluded from clustering features before processing.
    """
    # Validate KPI exclusion to prevent circularity in peer group construction
    validate_kpi_exclusion(cfg)
